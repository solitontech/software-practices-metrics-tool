...
export const getDeepSearchPullRequests = (
  ...
  if (mappedKey === SEARCH_KEYS.TAGS) {
    ...
    mappedKey === SEARCH_KEYS.CREATION_DATE ||
    mappedKey === SEARCH_KEYS.CLOSED_DATE
  ) {
...
const filterByTags = (
  ...
  return pullRequests.filter((row) =>
    row.tags.some((tag: string) => tag.toLowerCase().includes(value)),
  );
};
...
const filterByVotesHistoryTimeline = (
  ...
  return pullRequests.filter((row) =>
    row.votesHistoryTimeline.some((vote) =>
      vote.author.toLowerCase().includes(value),
    ),
  );
};
...
const filterByDate = (
  ...
  return pullRequests.filter((row) => {
    const formattedValue = formatDateWithoutTime(
      row[mappedKey as keyof IPullRequestList] as string[],
    );
