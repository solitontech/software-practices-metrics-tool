...
import {
  ISquad,
  IUser,
} from "../../../contexts/clientFiltersContext/clientFiltersContextInterface";
...
export const ClientFilterContainer = () => {
  ...
  const onSquadChange = (
    ...
    const updatedFilters = produce(filters, (draftFilters: ISquad[]) => {
      const users = draftFilters[squadIndex][
        USERS[userKey] as keyof ISquad
    ...
    <div className={styles.filterContainer}>
      {!filters.length && (
        <>
          ...
          <div className={styles.noFiltersContent}>
            {" "}
            Configure filters in server-config.json
          </div>
        </>
      )}
      {filters.map((squad, squadIndex) => {
        ...
        const isAllDevelopersSelected = developerKeyValues.every(
          ([, developer]) => developer.isSelected,
        );
        const isAllReviewersSelected = reviewKeyValues.every(
          ([, reviewer]) => reviewer.isSelected,
        );
        ...
        return (
          <div
            className={`${styles.squadContainer} ${
              filters.length - 1 !== squadIndex ? styles.squadMargin : ""
            }`}
            key={squad.squadName}
          >
